...
- One root command (`./dev.sh`) starts FastAPI and Vite together; added in M2 with the frontend
...
| `GET /api/graph?repo=owner/repo&days=30&priority=main,stage,dev` | `git log --branches` with parents → lanes, nodes, edges (D1–D6) | `{lanes[], nodes[], edges[], authors[], summary, truncated}` |
...
| `GET /api/edge?repo=&source=&target=` | Edge details | see section 4 |
...
## 4. Frontend
**Top bar:** repo URL input · **Load** · **Refresh** (spinner, "fetched 2 min ago") · time-window selector · optional **branch priority** field (remembered per repo)
...
### M1 — backend (done)
- Modules: `config`, `git_runner` (isolated git, token as env-config header, redaction), `sync`,
  `gitlog`, `lanes` (pure lane logic), `graph`, `details`, `models`, `main`
- 47 tests pass (`uv run pytest`), ruff clean. The synthetic team repo covers branch-offs, a
  feature merged back, a PR merge of a deleted branch, a sync from dev into a feature, a squash
  commit (`(#12)`), a rename, a delete, and stage/main promotion
- Verified live against `yadavatul868/team-git-visualizer` (private): sync, refresh, graph,
  commit and edge details (PR #1 detected, 4 commits brought in); no token material anywhere in
  `.cache/`; a missing repo returns a clear 403 message
